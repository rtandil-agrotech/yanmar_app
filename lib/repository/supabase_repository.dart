...
class SupabaseRepository {
  ...
  Future<List<PlanProduksiModel>> getPlanProduksi({
    ...
    final result = await _client
        ...
        .filter(
            'production_plan_detail.production_actual.deleted_at', 'is', null)
        .order('order',
            ascending: true, referencedTable: 'production_plan_detail')
        .order('start_time', ascending: true)
        .order('recorded_time',
            ascending: true,
            referencedTable: 'production_plan_detail.production_actual');

    final List<PlanProduksiModel> planProduksiModel =
        result.map((e) => PlanProduksiModel.fromSupabase(e)).toList();

    return planProduksiModel;
  }

  RealtimeChannel subscribeToProductionActualChanges(
  ...
  Future getDelivery({
    ...
    final List<DeliveryPlanModel> deliveryPlanModel =
        result.map((e) => DeliveryPlanModel.fromSupabase(e)).toList();

    return deliveryPlanModel;
  }

  RealtimeChannel subscribeToItemRequestChanges(
      void Function(PostgresChangePayload) callback) {
    final subs = _client
  ...
  RealtimeChannel subscribeToChecklistChanges(
      void Function(PostgresChangePayload) callback) {
    final subs = _client
  ...
  Future<List<RackModel>> getRackList({required DateTime currentTime}) async {
    final result = await _client
        ...
        .select('id, rack_name, assembly_name, rack_placement')
        .isFilter('deleted_at', null)
        .order('rack_name', ascending: true)
        .order('rack_placement', ascending: true);

    final distinctRackName = result
        .map((e) => e['rack_name'] as String)
        .toSet()
        .toList()
    ...
    final DateTime endOfDay = DateTime(
        currentTime.year, currentTime.month, currentTime.day, 23, 59, 59, 999);

    for (int i = 0; i < distinctRackName.length; i++) {
      final opAssIdList = (result as List)
          .where((e) => e['rack_name'] == distinctRackName[i])
          .map((e) => e['id'])
          .toList();
      ...
      final planHeader = await _client
          ...
          .inFilter(
              'production_plan_detail.master_production_type_header.master_production_type_detail.master_parts.op_assembly_id',
              opAssIdList)
          .isFilter('deleted_at', null)
          .order('order',
              ascending: true, referencedTable: 'production_plan_detail')
      ...
      if (planHeader.isNotEmpty) {
        for (var details in planHeader[0]['production_plan_detail']) {
          (details['master_production_type_header']
                  ['master_production_type_detail'] as List)
              .removeWhere((element) => element['master_parts'] == null);
        }
      }

      final opAssDataList = result
          .where((e) => (e['rack_name'] as String) == distinctRackName[i])
          .toList();
  ...
  Future insertChecklist(int prodPlanHeaderId, int partId, int userId) async {
    final result = await _client
        ...
        .select('id')
        .eq('production_plan_header_id', prodPlanHeaderId)
        .limit(1);
    int id;
    ...
    final detail = SetChecklistDetailModel(
        headerId: id, partId: partId, checkedDoneTime: DateTime.now());

    await _client.from('checklist_detail').insert(detail.toJson());

    // Update all check done if all parts from plan header are checked
    final allDetail = await _client
        .from('checklist_header')
        .select('id, checklist_detail(id, part_id)');

    final response = await _client.from('production_plan_header').select(
        'id, production_plan_detail(id, master_production_type_header(id, master_production_type_detail(id, part_id)))');

    final checklistParts = allDetail
        .expand((e) => e['checklist_detail'])
        .map((f) => f['part_id'])
        .toList();
    ...
    if (checklistParts.length == responseParts.length) {
      await _client
          .from('checklist_header')
          .update({'all_check_done_time': DateTime.now().toIso8601String()}).eq(
              'id', id);
    }
  }

  /* -------------------------- Monthly Plan Produksi ------------------------- */
  Future<List<MonthlyPlanProduksiModel>> getMonthlyPlanProduksi(
      {required DateTime startTime, required DateTime endTime}) async {
    final result = await _client
        .from('monthly_production_plan_header')
        .select(
            'id, start_time, end_time, monthly_production_plan_detail(id, master_production_type_header(id, type_name), production_qty, order)')
        .gte('start_time', startTime.toUtc().toIso8601String())
        .lte('end_time', endTime.toUtc().toIso8601String())
        .isFilter('deleted_at', null)
        .order('order',
            ascending: true, referencedTable: 'monthly_production_plan_detail')
        .order('start_time', ascending: true);

    final actuals = await _client
        .from('production_actual')
        .select(
            'id, production_plan_detail(id, master_production_type_header(id, type_name)), recorded_time')
    ...
    final List<MonthlyPlanProduksiModel> monthlyPlanProduksiModel = result
        .map((e) => MonthlyPlanProduksiModel.fromSupabase(e, actuals))
        .toList();
  ...
  Future<void> insertPlanProduksi(
      Map<String, dynamic> excelData, int createdBy) async {
    ...
    for (int i = 0; i < headersList.length; i++) {
      final UploadPlanProduksiHeaderModel header =
    ...
    for (int i = 0; i < detailsList.length; i++) {
      for (int j = 0; j < (detailsList[i]['zone'] as List).length; j++) {
        if (detailsList[i]['zone'][j] > 0) {
          ...
          if (prodTypeId.isEmpty)
            throw Exception(
                "Production Type ${detailsList[i]['model']} not found");

          final order = await _client
              .from('production_plan_detail')
              .select('id')
              .eq('header_id', headerIdList[j])
              .count(CountOption.exact);

          final UploadPlanProduksiDetailModel detail =
              UploadPlanProduksiDetailModel(
  ...
  Future<void> deleteMonthlyPlanProduksi({required int id}) async {
    await _client
        .from('monthly_production_plan_detail')
        .delete()
        .eq('header_id', '$id');
    await _client
        .from('monthly_production_plan_header')
  ...
  Future<void> uploadMonthlyPlanProduksi(List<Map<String, dynamic>> excelData,
      DateTime startTime, DateTime endTime, int createdBy) async {
    final headerId =
    ...
    for (int i = 0; i < excelData.length; i++) {
      ...
      if (prodTypeId.isEmpty)
        throw Exception("Production Type ${excelData[i].keys.first} not found");

      final order = await _client
          .from('monthly_production_plan_detail')
          .select('id')
          .eq('header_id', headerId.first['id'])
          .count(CountOption.exact);
  ...
  /* ------------------------------ UPLOAD MODEL ------------------------------ */
  Future<Map<String, dynamic>> getMasterProductionType(
    ...
    final List<MasterProductionTypeModel> models =
        result.map((e) => MasterProductionTypeModel.fromSupabase(e)).toList();
  ...
  Future<Map<String, dynamic>> getMasterProductionTypeDetail(
    ...
    final result = await _client
        ...
        .select(
            'id, master_parts(id, part_name, part_code, locator, master_op_assembly(id, assembly_name, rack_placement)), part_qty')
        .eq('header_id', typeId);

    final List<MasterProductionTypeDetailModel> details = result
        .map((e) => MasterProductionTypeDetailModel.fromSupabase(e))
        .toList();

    final MasterProductionTypeModel model =
        MasterProductionTypeModel.fromSupabase(header);

    return {'data': details, 'header': model};
  }

  Future<void> updateEstimatedProductionDuration(
      {required int typeId, required Duration productionDuration}) async {
  ...
  Future<void> insertMasterProductionType(
      List<Map<String, dynamic>> excelData, String modelName) async {
    ...
    modelId ??= await _client
        ...
        .insert({
          'type_name': modelName,
          'estimated_production_duration':
              getEstimatedProductionDuration(modelName)
        })
        .select('id')
        .limit(1)
        .then((value) => value.isNotEmpty ? value.first['id'] : null);
    ...
    for (var data in excelData) {
      ...
      await _client.from('master_production_type_detail').insert(
          {'header_id': modelId, 'part_id': partId, 'part_qty': data['qty']});
    }
  }
  ...
  Future<UserModel> getLoggedUser({required String uuid}) async {
    final result = await _client
        .from('users')
        .select('id, username, temp_user_roles(id, role_name), uuid')
        .isFilter('deleted_at', null)
        .eq('uuid', uuid)
        .limit(1);

    final userModel =
        result.map((e) => UserModel.fromSupabase(e)).toList().first;
